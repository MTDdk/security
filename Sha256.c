...
#define BYTE_POWER_OF_TWO 3 // used to convert number of bytes into number of bits (ex.: 64B = 512b = 64*2^3)
...
void digest(char* input, int inputLength, unsigned int* digest) {
    ...
    char * padded = calloc(paddingSize, sizeof(char)); // initialises an array filled with zero
...
int main(int argc, char **argv) {
    // check if something actually got put in
    if (argc < 2) {
